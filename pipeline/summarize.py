...
### STEP 3. 총 5개 뉴스를 아래 순서로 배열 (고정)
매일 첫 뉴스가 다양해지도록 손경제 큐레이션을 앞에 배치.
STEP 1의 손경제 3개 토픽을 [깊이 있는 경제뉴스] 원본 순서 T1·T2·T3라 할 때:
  - #1 = T3 (손경제 3번째 토픽)
  - #2 = T1 (손경제 1번째 토픽)
  - #3 = T2 (손경제 2번째 토픽)
  - #4 = STEP 2의 web_search 추가 뉴스 1
  - #5 = STEP 2의 web_search 추가 뉴스 2
web_search 추가 뉴스 2개(#4·#5) 사이 순서는 직장인 체감도 큰 것부터.
