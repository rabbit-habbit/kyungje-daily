...
### STEP 3. 총 5개 뉴스를 아래 순서로 배열 (고정 · 손경제와 web_search 교차)
STEP 1의 손경제 3개 토픽(원본 순서 T1·T2·T3)과 STEP 2의 web_search 뉴스 2개(W1·W2)를
아래처럼 교차 배치:
  - #1 = T1 (손경제 1번째 토픽)
  - #2 = W1 (web_search 추가 뉴스, 더 중요한 것)
  - #3 = T2 (손경제 2번째 토픽)
  - #4 = W2 (web_search 추가 뉴스, 나머지)
  - #5 = T3 (손경제 3번째 토픽)
W1·W2 사이 순서는 직장인 체감도 큰 것부터 (더 중요한 게 W1).
