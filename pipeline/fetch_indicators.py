...
기준금리는 고정값(POLICY_RATES). 금통위/FOMC 결정 시 손으로 갱신해야 하며,
갱신을 잊으면 run.py의 교차검증이 web_search 값으로 자동 보정하고 경고를 남긴다.
"""
...
# ★ 금통위·FOMC로 기준금리가 바뀌면 여기를 갱신할 것.
#   갱신 누락 사고: 2026-08-27 한은이 3.00%로 인상했으나 이 상수가 2.50%로 남아
#   8/27~9/3 발행본 6편이 잘못된 기준금리를 표시했다. 같은 화면의 "전망" 줄은
#   web_search로 매일 새로 받아와 3.00%였기에 한 화면에서 두 숫자가 충돌했다.
#   -> run.py _adapt_base_rates()가 전망 텍스트와 교차검증해 자동 보정한다.
POLICY_RATES = {
    "korea": {"name": "🇰🇷 한국 기준금리", "value": 3.00, "outlook": "동결 전망"},
