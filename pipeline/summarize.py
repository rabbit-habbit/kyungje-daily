...
- 급락·위기·손실 뉴스가 헤드라인인 날: **"ㅎㅎ"와 들뜬 이모지 금지.** 차분하고
  ...
  **단 마무리 느낌표는 이런 날에도 씁니다** (아래 형식 규칙 참고). 차분함과
  느낌표는 충돌하지 않습니다 - 마침표로만 끝내면 자동 발송처럼 건조해집니다
- 겁주거나 재촉하지 않는다. 클릭을 구걸하는 표현("꼭 보셔야 해요!!") 금지

형식:
- 2~3문장, **전체 120자 이내**. 짧을수록 좋다.
  ※ 이 멘트는 publ 그룹톡에 링크와 함께 나가는데, publ이 링크 포함 300자에서
    잘라버린다. 헤더+서명링크가 이미 159자를 쓰므로 멘트가 120자를 넘으면
    문장이 중간에 끊긴다 (8/21 사고: 145자 -> "천천히"에서 절단).
- **문장을 완결할 것.** 분량이 애매하면 한 문장을 통째로 빼서 줄인다.
...
- 느낌표는 멘트 전체에서 1~2개까지. "!!" 처럼 연달아 쓰거나 문장마다 붙이지 않습니다.
...
OUTPUT_SCHEMA = """\
{
  ...
  "group_note": "그룹톡 멘트 (2~3문장, 120자 이내, 느낌표로 종료, URL 없이, 그날 톤에 맞춰)"
}
"""
